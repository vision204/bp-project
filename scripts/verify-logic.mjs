...
// 이 마무리 일격이 남은 체력을 확실히 넘도록 열매 스텟을 크게 찍어둡니다
// (테스트 (4)와 같은 이유 — "막타 출처"만 검증하는 게 목적이라 정확한
// 스킬 데미지 수치는 중요하지 않습니다. 스킬 너프로 기본 스텟(0)에서는
// 근소한 차이로 못 죽일 수 있어 추가함).
pMix.stats.fruit = 3000;
recomputeDerivedStats(pMix);
